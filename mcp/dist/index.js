...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'db_query': {
                ...
                if (!isReadOnly && !query.toLowerCase().includes('returning')) {
                    throw new McpError(ErrorCode.InvalidRequest, 'Only read queries or mutations with RETURNING clause are allowed');
                }
        ...
        const pgError = error;
        throw new McpError(ErrorCode.InternalError, `Database error: ${pgError.message}`, {
            code: pgError.code,
