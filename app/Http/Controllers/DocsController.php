...
class DocsController extends Controller
{
    ...
    public function fluxoDocumentos()
    {
        $filePath = base_path('docs/FLUXO-DOCUMENTO-COMPLETO.md');
        ...
        // Cache baseado na última modificação do arquivo
        $lastModified = File::lastModified($filePath);
        $cacheKey = 'docs.fluxo-documentos.' . $lastModified;
        ...
        $fileInfo = [
            'lastModified' => date('d/m/Y H:i:s', $lastModified),
            'size' => number_format(File::size($filePath) / 1024, 2) . ' KB',
            'path' => 'docs/FLUXO-DOCUMENTO-COMPLETO.md'
        ];
        
        return view('docs.fluxo-documentos', compact('htmlContent', 'fileInfo'));
    }
}
